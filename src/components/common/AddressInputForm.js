...
const AddressInputForm = (props) => {
  ...
  const [provinceId, setProvinceId] = useState(props.provinceId ? props.provinceId : '0')
  const [districtId, setDistrictId] = useState(props.districtId ? props.districtId : '0')
  ...
  const showProvinces = provinces.map((province, index) => <option
    key={province.id}
  ...
  const showDistricts = districts.map((district, index) => <option
    key={district.id}
  ...
  const showWards = wards.map((ward, index) => <option
    key={ward.id}
    value={ward.id}
  >{ward.name}</option>)
    ...
    <div style={{ display: 'flex', justifyContent: 'center' }}>
      <div style={{ display: 'inline-block', marginTop: '20px', padding: '10px' }}>
        ...
        <div className="input-field">
          ...
          <select value={provinceId} className="cbbox !important" onChange={handleProvinceInputChange}>
            <option
              value="0"
        ...
        <div className="input-field" >
          ...
          <select value={districtId} className="cbbox !important" onChange={handleDistrictInputChange}>
            <option
              value="0"
        ...
        <div className="input-field" >
          ...
          <select value={wardId} className="cbbox !important" onChange={handleWardInputChange}>
            <option
              value="0"
        ...
        <div className="input-field" >
          ...
          {props.tag!=='1' ? <Button style={{width:'100%'}} variant="contained" color="primary" onClick={handleSubmit}>
            Cập nhật
            </Button> : <div style={{display:'flex',flexDirection:'row'}}>
            <Button style={{width:'100%'}} variant="contained" color="primary" onClick={handleSubmit}>
            Cập nhật
            </Button>
            <Button  style={{width:'100%'}} variant="contained" onClick={() => props.onClick()}>
            Hủy
            </Button>
            </div>
          }
