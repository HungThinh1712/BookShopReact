...
const SearchPage = (props) => {
  ...
  const searchedBooks = useSelector(state => state.books.searchedResultBooks.entities ? state.books.searchedResultBooks.entities: [] )
  ...
  const total  = useSelector(state=>state.books.searchedResultBooks.total ? state.books.searchedResultBooks.total: 0 )
    ...
    <div >
      ...
      <div >
        <div className={classes.drop_down}  >
          <DropDown label='Thể loại' handleChange={handleTypeChange} data={types} />
        ...
        {searchedBooks.length >2 ? <Paper className={`cover_container_searchpage ${classes.container}`}>
          {showBooks}
        </Paper> : <Paper className={`cover_container_searchpage ${classes.container}`}>
          {showBooks}
          <div>
          </div>
          <div></div>
