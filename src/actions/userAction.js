...
export const getAdminUsersRequest =  (name) => async (dispatch) => {
    const url = CallApis.API_URL.concat(`/Users/Admin/GetAdminsUser?name=${name}`)
    dispatch(backdropAction.setOpenBackDrop);
...
export const deactivateOrActivate =  (id,isActivate) => async (dispatch) => {
    const url = CallApis.API_URL.concat(`/Users/Admin/Activate?id=${id}&isActivate=${isActivate}`)
    await axios.get(url)
        .then(res => {
